...
Xs_dummy <- Xs
Xs_dummy[[1]][1,1] <- NA
params_dummy <- new("CoupledMWCAParams", Xs=Xs_dummy)
...
Ms[[2]][] <- 1
Ms[[3]][] <- 1
params_dummy <- new("CoupledMWCAParams", Xs=Xs, mask=Ms)
...
params_dummy <- new("CoupledMWCAParams", Xs=Xs,
    common_initial=list(A1=A1, A2=A2_dummy, A3=A3, A4=A4, A5=A5))
expect_error(CoupledMWCA(params_dummy))

# Test algorithms
params_dummy <- new("CoupledMWCAParams", Xs=Xs,
    common_algorithms=list(A1="mySVDD", A2="myALS_SVD", A3="myNMF",
            A4="myICA", A5="myCX"))
expect_error(CoupledMWCA(params_dummy))
...
# Test iteration
params_dummy <- new("CoupledMWCAParams", Xs=Xs,
    common_iteration=list(A1=1.1, A2=2, A3=3, A4=4, A5=5))
expect_error(CoupledMWCA(params_dummy))

# Test initial/fix
params_dummy <- new("CoupledMWCAParams", Xs=Xs,
    common_initial=list(A1=A1, A2=A2, A3=A3, A4=A4, A5=A5),
    common_fix=list(A1=TRUE, A2=FALSE, A3=TRUE, A4=FALSE, A5=TRUE))
...
params_dummy <- new("CoupledMWCAParams", Xs=Xs,
    common_initial=list(A1=A1, A2=A2, A3=A3, A4=A4, A5=A5),
    common_decomp=list(A1=FALSE, A2=TRUE, A3=FALSE, A4=TRUE, A5=FALSE))
out_dummy <- CoupledMWCA(params_dummy)
...
# Test dims
params_dummy <- new("CoupledMWCAParams", Xs=Xs,
    common_dims=list(A1=300, A2=3, A3=5, A4=4, A5=4))
expect_error(CoupledMWCA(params_dummy))

# Test transpose
params_dummy <- new("CoupledMWCAParams", Xs=Xs,
    common_transpose=list(A1=FALSE, A2=TRUE, A3=FALSE, A4=TRUE, A5=FALSE))
expect_error(expect_error(CoupledMWCA(params_dummy)))
...
# Test specific
expect_error(expect_error(new("CoupledMWCAParams", Xs=Xs, specific=TRUE)))
expect_error(expect_error(new("CoupledMWCAParams", Xs=Xs, specific=FALSE)))
expect_error(new("CoupledMWCAParams", Xs=Xs, specific="TRUE"))

# Test coretype
params_dummy <- new("CoupledMWCAParams", Xs=Xs, common_coretype="TUCKER")
expect_error(CoupledMWCA(params_dummy))

# Test thr
params_dummy <- new("CoupledMWCAParams", Xs=Xs, thr=1E+100)
